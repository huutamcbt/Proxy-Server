...
class ProxyServer:
    ...
    def connectProxy(self):
        # create the address of the next proxy server
        proxyServer = webserver.WebServer()
        proxyServer.webServer = input("Type your proxy server address: ")
        proxyServer.port = int(input("Type your proxy server port: "))
        ...
        while config["PERMISSION"]:
            ...
            process = threading.Thread(target= self.transferData, args= (client, proxyServer))
            process.daemon = True
            process.start()
        

    def transferData(self, client, proxyServer):
        # create a web server variable
        server = webserver.WebServer()

        # receive the HTTP request from browser
        request = client.clientSocket.recv(config["BUFFER_SIZE"])

        # extract url
        url = self.extractURL(request)

        # get the IP address and port of web server
        server.webServer, server.port = self.getWebServerAddr(url)
        ...
        if(server.isServerDenied() == True):
            ...
            self.connectClientToProxy(client, proxyServer, request)

        client.clientSocket.close()



    def connectClientToProxy(self, client, proxyServer, request):
        # extract a method
        method = self.getMethod(request)

        # create a socket to connect to the proxy server
        sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        sock.connect((proxyServer.webServer, proxyServer.port))

        # send a request from a browser to the proxy server
        sock.sendall(request)
    ...
    def transferCONNECTMethod(self, client, sock):
        # create a response from a proxy
        response = sock.recv(config["BUFFER_SIZE"])

        # extract the first line in a response
        first_line = response.decode().split("\n")[0]

        # assign empty value to response string
        print(first_line)
        if("HTTP/1.1 200" in response.decode()):
            client.clientSocket.sendall(self.create200Response().encode())
            client.clientSocket.setblocking(0)
            sock.setblocking(0)
